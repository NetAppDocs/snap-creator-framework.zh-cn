...
NOTE: 有关 Snap Creator 通用错误的信息，请参阅link:../administration/index.html["《 Snap Creator Framework 4.3.3 管理指南》"]。
...
** 在 Windows 主机上，必须将 IBM Domino 安装路径添加到 PATH 环境变量中。欲了解更多信息，请参阅link:https://docs.netapp.com/us-en/snap-creator-framework/installation/task_configure_windows_specific_settings_adding_path_to_the_environment_variables.html["Snap Creator Framework安装指南 - 将路径添加到环境变量"]。
...
默认情况下，如果一个或多个数据库处于不一致或损坏状态，则备份操作将失败。您可以强制插件在出现错误时继续进行备份操作，如步骤 link:task_using_the_gui_to_create_a_configuration_file.html["使用 GUI 创建配置文件的任务"] 中所述。
...
Domino 插件无法与 snap_timestamp_only = N 结合使用snap_timestamp_only 应设置为 Y
 a| 
有关详细信息，请参见步骤 link:task_using_the_gui_to_create_a_configuration_file.html["使用 GUI 创建配置文件的任务"]。
