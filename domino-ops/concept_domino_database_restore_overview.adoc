...
卷还原和单文件还原通常具有不同的目标目录。您可以在备份和还原配置文件的 Data Restore Directory 参数中指定目标目录（请参阅步骤link:task_using_the_gui_to_create_a_configuration_file.html["使用 GUI 创建配置文件的任务"]）：
